...
## 0. RAPOR FARKI: ÖTR'de RV32IMFC, gerçekte RV32IMC

### Durum

Ön Tasarım Raporu (ÖTR) §3.6.2'de çekirdek komut seti **RV32IMFC**
olarak beyan edilmişti; **F** harfi tek duyarlıklı kayan nokta
(floating-point) eklentisini ifade eder.

Teslim edilen tasarımda **F eklentisi YOKTUR**. Gerçek komut seti
**RV32IMC**'dir.

Kanıt — `rtl/Memory/soc_top.sv` içindeki çekirdek örneklemesi:
...
1. **Uygulama ihtiyacı yok.** TFLite Micro Speech modeli
   tamamen **INT8 tamsayı** aritmetiğiyle çalışır (EK-1: "1960 adet
   INT8 spektrogram değeri"). Softmax dahil tüm hesaplama NPU
   içinde Q0.12 sabit noktada yapılır. Ne firmware ne NPU tek bir
   kayan nokta buyruğu kullanır.

2. **Alan ve zamanlama maliyeti.** CV32E40P'nin FPU'su tasarımın
   en büyük tek bloğu olurdu. 46 KB SRAM zaten die alanının
   büyük kısmını tutuyor (23 makro, 6.544.380 µm²); FPU eklemek
   hem alanı hem kritik yolu belirgin biçimde büyütürdü.

3. **Şartname F eklentisini istemiyor.** §4.2.2.1 yalnızca
   "CV32E40P RISC-V çekirdek IP kullanılacaktır" der; hangi
   konfigürasyonun kullanılacağının "DDK tarafından
   netleştirileceği" yazılıdır. Zorunlu bir F isteri yoktur.

### Nerede doğru beyan ediliyor

`asic/README.md` satır 13 gerçeği yazar:

    CV32E40P (RV32IMC, FPU kapalı)

`asic/results/config/resolved.json` ve sentez raporları da
FPU'suz yapılandırmayı yansıtır; teslim edilen çıktılarla RTL
arasında **çelişki yoktur**.

### Sonuç

Bu bir tasarım eksikliği değil, **ÖTR'deki bir fazla beyandır**.
Final teslimde doğru ISA (RV32IMC) belgelenmiştir. Sunumda ÖTR'ye
atıf yapılırsa bu fark belirtilmelidir.
