...
`ifdef UVM_AXI
    ...
    // -----------------------------------------------------------------------
    // IKINCI UVM AGENT - SoC ANA YOLU  (12 Eylul 2026)
    //
    // NEDEN EKLENDI
    //   Mevcut `npu_eng_if` NPU motorunun IC arayuzune baglidir ve
    //   yalnizca TCM'i gorur (0x0 - 0x76bc). SoC bolge kapsami
    //   (bootrom, gpio, uart, i2c, qspi...) orada OLCULEMEZ - motorun
    //   o adreslere erismesi FIZIKSEL OLARAK mumkun degildir.
    //
    //   `merged_m_*` CPU, DMA ve JTAG master'larinin BIRLESTIGI ve
    //   interconnect'e giren noktadir; tum SoC trafigi buradan gecer.
    //   (soc_top.sv:449-465 - axi_lite_interconnect m_* portlari)
    //
    // SAAT/RESET ADI UYARISI
    //   Ilk agent'ta yapilan hata burada tekrarlanmamalidir: tb icinde
    //   sinyal adlari clk/rst_n'dir, uut PORT adlari clk_i/rst_ni'dir.
    //   Yanlis ad ORTUK WIRE yaratir, saat toggle etmez ve monitor
    //   SESSIZCE sifir islem yakalar (derleme hatasi vermez).
    // -----------------------------------------------------------------------
    axil_if soc_bus_if (.clk(clk), .rst_n(rst_n));

    assign soc_bus_if.awaddr  = uut.merged_m_awaddr;
    assign soc_bus_if.awvalid = uut.merged_m_awvalid;
    assign soc_bus_if.awready = uut.merged_m_awready;
    assign soc_bus_if.wdata   = uut.merged_m_wdata;
    assign soc_bus_if.wstrb   = uut.merged_m_wstrb;
    assign soc_bus_if.wvalid  = uut.merged_m_wvalid;
    assign soc_bus_if.wready  = uut.merged_m_wready;
    assign soc_bus_if.bresp   = uut.merged_m_bresp;
    assign soc_bus_if.bvalid  = uut.merged_m_bvalid;
    assign soc_bus_if.bready  = uut.merged_m_bready;
    assign soc_bus_if.araddr  = uut.merged_m_araddr;
    assign soc_bus_if.arvalid = uut.merged_m_arvalid;
    assign soc_bus_if.arready = uut.merged_m_arready;
    assign soc_bus_if.rdata   = uut.merged_m_rdata;
    assign soc_bus_if.rresp   = uut.merged_m_rresp;
    assign soc_bus_if.rvalid  = uut.merged_m_rvalid;
    assign soc_bus_if.rready  = uut.merged_m_rready;

    // -----------------------------------------------------------------------
    // X KAYNAGI TANISI (gecici)
    // El sikismada rdata X cikiyor. Hangi slave'in rdata'si X?
    // -----------------------------------------------------------------------
    always @(posedge clk) begin
        if (rst_n && soc_bus_if.rvalid && soc_bus_if.rready &&
            $isunknown(soc_bus_if.rdata)) begin
            $display("  [XTANI] t=%0t m_rdata=0x%08h read_sel_q=%0d",
                     $time, soc_bus_if.rdata, uut.u_interconnect.read_sel_q);
            $display("          s0=0x%08h s1=0x%08h s2=0x%08h s5=0x%08h",
                     uut.s0_rdata, uut.s1_rdata, uut.s2_rdata, uut.s5_rdata);
            $display("          s6=0x%08h s10=0x%08h s11=0x%08h s12=0x%08h",
                     uut.s6_rdata, uut.s10_rdata, uut.s11_rdata, uut.s12_rdata);
            $display("          rvalid: s0=%0b s1=%0b s2=%0b s5=%0b s6=%0b s10=%0b",
                     uut.s0_rvalid, uut.s1_rvalid, uut.s2_rvalid,
                     uut.s5_rvalid, uut.s6_rvalid, uut.s10_rvalid);
        end
    end
...
`endif
    ...
    //
    // 12 Eylul 2026: IKI ARAYUZ birden sayilir.
    //   Ikinci agent (soc_bus_if) eklendikten sonra monitor'un statik
    //   sayaclari IKI noktanin toplamini tutar. Ham sayim yalniz
    //   npu_eng_if'i sayarsa capraz kontrol HAKSIZ YERE kalir.
    //   (Ilk kosumda tam bu oldu: ham 162.048 vs monitor 382.691.)
