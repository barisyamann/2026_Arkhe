...
seed=20260906;rng=random.Random(seed);noise=np.random.default_rng(seed)
z=zipfile.ZipFile(r/'closure_20260905/mini_speech_commands.zip')
previous={x[0] for x in json.loads((r/'closure_20260905/selection.json').read_text())['speech']}
...
for label,words in [(2,['yes']),(3,['no']),(1,['down','go','left','right','stop','up'])]:
    pool=[n for n in files if n.split('/')[-2] in words and n not in previous]
    selection += [(n,label) for n in rng.sample(pool,400)]
# Selection is frozen BEFORE consulting either model. Silence is a separate synthetic stratum.
(d/'selection.json').write_text(json.dumps({'seed':seed,'speech':selection,'synthetic_silence':100, 'stress_tensors':100, 'excluded_previous_speech':len(previous)},indent=2))
...
for index in range(1400):
    if index<1200:
        ...
        name=f'synthetic_silence_{index-1200}';label=0
        samples=np.zeros(16000) if index==1200 else noise.integers(-2,3,16000).astype(np.float64)
        q=ozellik_cikar(samples).reshape(-1);source_hash=hashlib.sha256(samples.astype('<i2').tobytes()).hexdigest()
    else:
        k=index-1300;name=f'stress_{k}';label=None;source_hash=None
        if k<5:q=np.full(1960,[-128,-1,0,1,127][k],dtype=np.int8)
        elif k<7:q=np.resize(np.array([-128,127] if k==5 else [-1,1],dtype=np.int8),1960)
        elif k<17:
            q=np.full(1960,-128,dtype=np.int8);q[(k-7)*195]=127
        else:q=noise.integers(-128,128,1960,dtype=np.int16).astype(np.int8)
...
metadata={'source':'https://www.tensorflow.org/tutorials/audio/simple_audio',
    'archive_sha256':hashlib.sha256((r/'closure_20260905/mini_speech_commands.zip').read_bytes()).hexdigest(),
    'model_sha256':hashlib.sha256((r/'model/micro_speech_quantized.tflite').read_bytes()).hexdigest(),
    'seed':seed,'speech_count':1200,'synthetic_silence_count':100,'stress_count':100,
    'limitations':['Stratified evaluation excluding the earlier 75 speech files, not entire official test set; training overlap unknown',
