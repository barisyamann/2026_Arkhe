...
int main(void)
{
    ...
    *TIM_ENA = 1u;
    ...
    bas = (int32_t)*TIM_CNT;
    for (i = 0; i < 4; i++) fc_acc[i] = tcm_i32(OFS_FC_B, i);
    ...
    int32_t logits[4], mx=-128;uint32_t exps[4],den=0; volatile uint32_t probs[4];
    for(i=0;i<4;i++){int32_t q=multiply_quantized(fc_acc[i],FC_MULT,FC_RSHIFT)+FC_ZERO;
        logits[i]=q < -128 ? -128 : (q>127?127:q);if(logits[i]>mx)mx=logits[i];}
    for(i=0;i<4;i++){int diff=mx-logits[i];exps[i]=diff==0?4096:TCM[OFS_LUT+diff];den+=exps[i];}
    for(i=0;i<4;i++)probs[i]=(exps[i]<<12)/den;
    son   = (int32_t)*TIM_CNT;
    gecen = son - bas;
    ...
    for(i=0;i<4;i++)TCM[OFS_SONUC+6+i]=probs[i];
