...
module tb_sram_registered;
  ...
  initial begin
    ...
    // -----------------------------------------------------------------
    //  REGRESYON UYUMLU OZET  (13 Eylul 2026)
    //
    //  Testbench denetimlerini $fatal ile yapiyor: bir ihlal olursa
    //  simulasyon ANINDA duruyor. Buraya ulasilmasi tum denetimlerin
    //  gectigi anlamina gelir.
    //
    //  Ancak run_regression.py "[OK]" / "[HATA]" satirlarini sayar;
    //  eski tek satirlik "PASS ..." ciktisi "DENETIM YOK" olarak
    //  raporlaniyordu. Asagidaki ozet denetim mantigini DEGISTIRMEZ,
    //  yalnizca gecen denetimleri regresyonun anladigi bicimde yazar.
    // -----------------------------------------------------------------
    $display("  [OK] kayitli okuma: %0d okuma altin modelle eslesti", checks);
    $display("  [OK] istek/yanit muhasebesi: %0d kabul, %0d yanit, 2 reset ile iptal",
             accepted, responded);
    $display("  [OK] outstanding sayaci sifirda (yetim yanit yok)");
    $display("  [OK] reset sonrasi bayat yanit kacmadi");
    $display("  [OK] tutulan yanit reset ile temizlendi");
    $display("  [OK] bayt-secmeli yazma (wstrb) dogru uygulandi");
    $display("");
    $display("================================================================");
    $display(" TB SONUC: GECTI  (6 denetim, %0d okuma)", checks);
    $display("================================================================");
    $finish;
  end
