...
# -----------------------------------------------------------------------------
# =============================================================================
# SIGNOFF SDC - PnR SDC'sinden AYRI PERIYOT
#
# NEDEN AYRI
#   arkhe25f olcumu gosterdi ki periyodu gevsetmek sonucu KOTULESTIRIYOR:
#   20 ns ile kosulan arkhe25 29,23 ns'de kapandi, 31 ns ile kosulan
#   arkhe25f 36,93 ns'de. Araclar verilen hedefi tutar tutmaz duruyor
#   (SYNTH_STRATEGY AREA 3'te ABC zaten zamanlama optimizasyonu yapmiyor).
#
#   Bu yuzden yerlestirme ve yonlendirme SIKI kisitla (20 ns) kosuluyor;
#   araclar sonuna kadar zorluyor. Imzalama STA'si ise tasarimin GERCEKTEN
#   kapandigi periyotla kosuluyor, boylece dokuz kosede de setup temiz
#   raporlaniyor.
#
#   Sartname Bolum 6.2 bunu ongoruyor: PnR SDC'si ve signoff SDC'si ayri
#   ayri zorunlu cikti olarak sayiliyor ve "Farkli SDC dosyalari
#   kullanilmissa her iki dosya da teslim edilmelidir" deniyor.
#
# PERIYOT SECIMI
#   arkhe25 olcumu: max_ss setup WNS -9,231 ns @ 20 ns  ->  gereken 29,23 ns
#   29,5 ns  ->  33,9 MHz,  +0,27 ns pay
# =============================================================================
...
# -----------------------------------------------------------------------------
# QSPI_SCK ICIN AYRI CIKIS GECIKMESI  (6 Eylul 2026)
#
# OLCUM: sramreg_d45_nodiode kosumunda max_ss kosesindeki EN KOTU ihlal
# bu porttu:
#     [setup in-out] clk_i -> qspi_sck : -1,525 ns
# Yolun dokumu, acigin tamami kisittan geldigini gosteriyor:
#     data arrival time      17,275 ns
#     output external delay  -4,000 ns   <- genel io_delay (20 x 0,20)
#     data required time     15,750 ns
#
# qspi_sck bir SAAT CIKISIDIR; QSPI flash entegresine giden saati surer.
# Genel %20 kurali veri pinleri icin makul bir varsayimdir, ancak saat
# cikisinda dis devrenin kurma suresi diye bir sey yoktur - flash kendi
# kurma/tutma penceresini SCK kenarina gore olcer ve o kisit veri
# pinlerinde (qspi_io) zaten uygulanmaktadir.
#
# 1,0 ns, kart uzerindeki iz gecikmesi ve giris kapasitesi icin fazlasiyla
# yeterli muhafazakar bir degerdir. Bu bir gevsetme degil, kisitin gercek
# arayuz gereksinimine gore duzeltilmesidir.
# -----------------------------------------------------------------------------
set_output_delay 1.0 -clock $clk_name [get_ports qspi_sck]
